...
export default {
  ITEM_NAMES: [
    ...
    // Добавьте остальные наименования товаров
  ],

  generateMockData() {
    const rowData = [];
    for (let i = 0; i < 200; i++) {
      const name = this.ITEM_NAMES[i % this.ITEM_NAMES.length];
      const proficiency = (Math.random() * 100).toFixed(2);
      const skillCoefficient = (Math.random() * 100).toFixed(2);
      const cnt = Math.floor(Math.random() * 100);
      const inv = `${Math.floor(Math.random() * 1000)}/${Math.floor(Math.random() * 1000)}`;
      const orderDate = new Date(2024, Math.floor(Math.random() * 12), Math.floor(Math.random() * 28));
      const quantity = Math.floor(Math.random() * 100);
      const buy = Math.random() < 0.5;
      const purchase = (Math.random() * 20).toFixed(2);
      const sale = (Math.random() * 20).toFixed(2);
      const steamPrice = (Math.random() * 20).toFixed(2);

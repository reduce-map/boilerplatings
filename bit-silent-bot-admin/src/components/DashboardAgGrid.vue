...
import {
  formatDateTime,
  getAllCurrenciesFromPrioritizedCountries,
  getFlagByCurrency,
} from "../utils";
...
const generateCellClassRules = (fieldName: string) => {
  return {
    "cell-changed": (params: { data: Record<string, any> }) =>
      params.data?.[`${fieldName}Changed`],
  };
};
...
const columnDefs = ref<ColDef[]>([
  ...
  // TEAM: not sure if we need this because of export import export workflow
  // {
  //   headerName: '',
  //   valueGetter: () => '/',
  //   editable: false,
...
function checkboxCellRenderer(params: any) {
  return params.value ? "✅" : "❌";
}

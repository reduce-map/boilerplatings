...
<template>
  ...
  <div class="content-center" v-if="loading">
    <Spin size="large" />
  </div>
  <div v-else>
    ...
    <div v-else>
      <Timeline>
        <TimelineItem v-for="message in chatMessages" :key="message.id">
          ...
          <p class="content" v-line-clamp="2">{{ message.text }}</p>
        </TimelineItem>
      </Timeline>
      <div class="content-center">
        <Spin size="large" v-if="loadingMore" />
        <Button v-else @click="fetchMoreChatMessages" type="dashed"
          >Load more</Button
        >
      </div>
    </div>

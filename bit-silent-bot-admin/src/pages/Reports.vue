...
const exportFileName = "reports-trading";
...
const exportData = () => {
  gridApi.value?.exportDataAsCsv({
    ...
    fileName: `${exportFileName}-${formatDateTime()}`,
  });
};
...
const handleFileUpload = (file: File): boolean => {
  ...
  reader.onload = (event: ProgressEvent<FileReader>) => {
    ...
    const jsonData: any[][] = XLSX.utils.sheet_to_json(worksheet, {
      header: 1,
    }) as any[][]; // Asserting type as array of arrays

    const rowData: TradingHistoryRecord[] = jsonData
      .slice(1)
      .map((row: any[]): TradingHistoryRecord => {
